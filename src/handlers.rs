...
async fn require_auth(headers: &HeaderMap, state: &SharedState) -> Result<String, StatusCode> {
...
async fn change_password(
        ...
        .change_password(&username, &req.old_password, &req.new_password)
    {
        ...
        _ => (
            ...
            Json(serde_json::json!({ "error": "Invalid old password" })),
        )
            .into_response(),
    }
}
...
pub fn channel_routes() -> axum::Router<SharedState> {
    axum::Router::new()
        .route(
            ...
            axum::routing::post(toggle_channel),
        )
        .route(
            "/api/channels/:id",
...
async fn list_logs(
    ...
    match s.db.list_logs(limit, offset, None, None, None) {
        Ok((logs, total)) => {
            Json(serde_json::json!({ "logs": logs, "total": total, "page": page, "limit": limit }))
                .into_response()
        }
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub fn dashboard_routes() -> axum::Router<SharedState> {
    axum::Router::new()
        ...
        .route(
            "/api/dashboard/tree",
            axum::routing::get(channel_tree_stats),
        )
...
async fn query_model_tag(Query(q): Query<QueryModelTag>) -> impl IntoResponse {
    match crate::models::find_model_tag(&q.model) {
        ...
        None => (
            StatusCode::NOT_FOUND,
            Json(serde_json::json!({ "error": "Model not found in tag database" })),
        )
            .into_response(),
    }
}
...
pub fn channel_action_routes() -> axum::Router<SharedState> {
    axum::Router::new()
        ...
        .route(
            "/api/channels/:id/test-model",
            axum::routing::post(test_model),
        )
        .route(
            "/api/channels/:id/test-embedding",
            axum::routing::post(test_embedding),
        )
        .route(
            "/api/channels/:id/discover",
...
async fn test_model(
    ...
    let model_type = model_tag
        .as_ref()
        .map(|t| t.model_type.clone())
        .unwrap_or(crate::models::ModelType::Chat);
    ...
    match result {
        Ok(resp) => {
            ...
            if let Some(tag) = model_tag {
                response_json["context_window"] =
                    serde_json::Value::Number(tag.context_window.into());
...
pub fn chat_test_routes() -> axum::Router<SharedState> {
    axum::Router::new().route("/api/chat/test", axum::routing::post(chat_test))
}
...
async fn chat_test(
    ...
    let channel = channels.iter().filter(|c| c.enabled).find(|c| {
    ...
    match channel {
        ...
        None => (
            StatusCode::NOT_FOUND,
            Json(serde_json::json!({ "error": "No available channel for this model" })),
        )
            .into_response(),
    }
}
...
pub fn settings_routes() -> axum::Router<SharedState> {
    axum::Router::new()
        ...
        .route(
            "/api/model-breakers",
            axum::routing::get(get_model_breakers),
        )
        .route(
            "/api/model-breakers/reset",
            axum::routing::post(reset_model_breaker),
        )
}
...
async fn reset_model_breaker(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Json(req): Json<ResetModelBreakerRequest>,
) -> impl IntoResponse {
    let _ = require_auth(&headers, &state).await;
    let s = state.read().await;
    s.breaker
        .reset_model_breaker(&s.db, &req.channel_id, &req.model)
        .await;
    Json(serde_json::json!({ "success": true })).into_response()
}
...
async fn chat_completions(
    ...
    if !keys.is_empty() {
        ...
        match key_str {
            Some(k) => match s.db.validate_api_key(k) {
                Ok(Some(api_key)) => {
    ...
    let resolved_entries: Vec<ApiEntry> = if req.model.eq_ignore_ascii_case("auto") {
        ...
        let matched: Vec<_> = entries
            ...
            .filter(|e| {
                futures::executor::block_on(s.breaker.is_model_available(
                    &s.db,
                    &e.channel_id,
                    &e.model,
                ))
    ...
    body_map.insert(
        "messages".to_string(),
        serde_json::Value::Array(req.messages.clone()),
    );
    if req.stream {
        body_map.insert("stream".to_string(), serde_json::Value::Bool(true));
    }

    for (attempt, entry) in resolved_entries
        .iter()
        .cycle()
        .enumerate()
        .take(max_retries)
    {
        ...
        if !futures::executor::block_on(s.breaker.is_model_available(
            &s.db,
            &channel.id,
            &req.model,
        )) {
                ...
                if resp.status().is_success() {
                    ...
                    if req.stream {
                        ...
                        let (prompt_tokens, completion_tokens) =
                            parse_stream_tokens(&String::from_utf8_lossy(&body));
                        tokio::spawn(async move {
                            let s2 = state2.read().await;
                            s2.breaker
                                .record_model_success(&s2.db, &channel_id, &model)
                                .await;
                    ...
                    let state2 = state.clone();
                    let channel_id = channel.id.clone();
                    let _channel_name = channel.name.clone();
                    let model = req.model.clone();

                    tokio::spawn(async move {
                        let s2 = state2.read().await;
                        s2.breaker
                            .record_model_failure(&s2.db, &channel_id, &model)
                            .await;
